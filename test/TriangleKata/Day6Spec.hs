module TriangleKata.Day6Spec (spec) where

    import Test.Hspec
    import TriangleKata.Day6 (triangle, TriangleType(..))
    ...
    spec = do
        ...
        it "isosceles triangle has last two sides equal" $ do
            triangle (15, 10, 10) `shouldBe` Isosceles

        it "isosceles triangle has the first and the last sides equal" $ do
            triangle (8, 10, 8) `shouldBe` Isosceles
        ...
        it "illegal triangle has sum of the first and the last sides less or equal to the second one" $ do
            ...
            triangle (7, 13, 6) `shouldBe` Illegal
